"""network2_for_submit.py
~~~~~~~~~~~~~~
network2.py for submission
1. epoch is excluded outsilde the class.
2. predict func is added.
"""
...
class Network():
    ...
    def accuracy(self, data, convert=False):
        ...
        if convert:
            ...
            results = [(np.argmax(self.feedforward(x)), y)
                       for (x, y) in data]
        # Add self.pred_Y_data for evaluation
    ...
    def predict(self, data):
        ...
        for i in range(0, len(data)):
            # print(len(data[i]))
            results.append(np.argmax(self.feedforward(data[i])))

        self.prd_Y_data = np.array(results)
    ...
    def total_cost(self, data, lmbda, convert=False):
        ...
        # change here if you want to use L1 reguralization
        return cost
...
def vectorized_result(j):
    ...
    """
    # (2,1) arrays for click or non-click
    e = np.zeros((2, 1))
    e[j] = 1.0
    return e
