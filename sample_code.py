...
def evaluation_func(true_surv, predicted_surv, TF_list):
    ...
    if TF_list[2]:
        ...
        print classification_report(
            true_surv, predicted_surv, target_names=target_names)

    # plots precision-recall curve
    if TF_list[3]:
        precision, recall, pr_thresholds =\
            precision_recall_curve(true_surv, predicted_surv)
...
#     'Name', 'Sex', 'Ticket', 'Cabin', 'PassengerId',
#     'Survived', 'Embarked'], axis=1)
train_df = train_df[['Age', 'Pclass', 'Fare', 'Gender']]
# print train_df.head()
# axes = pd.tools.plotting.scatter_matrix(train_df, alpha=0.2)
# plt.tight_layout()
# plt.show()
...
# then predict to the test! Convert back to a numpy array
...
predictions_file = open("EstimationResult.csv", "w")
